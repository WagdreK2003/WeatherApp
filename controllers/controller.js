...
// 5-day forecast from OpenWeather API (3-hour steps, aggregated by day on frontend)
const getForecast = async (req, res) => {
  try {
    ...
    const apiUrl = `https://api.openweathermap.org/data/2.5/forecast?q=${encodeURIComponent(city)}&appid=${WEATHER_API_KEY}&units=metric`;
    const response = await axios.get(apiUrl);
    const data = response.data;
    if (data.cod !== '200' && data.cod !== 200) {
      return res.status(404).json({ error: "City not found or forecast unavailable" });
    }
    res.status(200).json(data);
  } catch (err) {
    console.error(err);
...
module.exports = { addWeather, displayWeather, getAllWeather, bulkInsertWeather, getForecast };